       ...
       public async Task<IActionResult> GenerateQuiz([FromForm] GenerateQuizRequest request)
        {
            try
            {
                ...
                if (request.Level == 1)
                    levelInstruction = "כל השאלות חייבות להיות ברמה קלה (level 1).";
                else if (request.Level == 2)
                    levelInstruction = "כל השאלות חייבות להיות ברמה בינונית (level 2).";
                else if (request.Level == 3)
                    levelInstruction = "כל השאלות חייבות להיות ברמה קשה (level 3).";
                else
                    levelInstruction = "השאלות צריכות להיות בכל הרמות (קל, בינוני, קשה).";
                
                // Build additional instructions
                string additionalInstructions = "";
                if (!string.IsNullOrWhiteSpace(request.AdditionalInstructions))
                    additionalInstructions = $"הנחיות נוספות: {request.AdditionalInstructions}";
                ...
                var requestBody = new
                {
                    contents = new[]
                    {
                        new {
                            parts = new[] {
                                new { text = $@"צור {request.NumberOfQuestions} שאלות אמריקאיות בעברית על בסיס הטקסט הבא.
...
דריגות:
1. questionId - מספר סידורי מ-1 עד {request.NumberOfQuestions}
...
{extractedText.Substring(0, Math.Min(extractedText.Length, 5000))}"}
            ...
            {
                return new ObjectResult($"Internal Error: {ex.Message}") { StatusCode = 500 };
            }
        }
        // --- פונקציות עזר לחילוץ טקסט ---
        public async Task<string> ExtractTextFromPptxAsync(IFormFile file)
        {
